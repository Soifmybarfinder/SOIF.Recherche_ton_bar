...
let isSurpriseMode = false; // État du mode "Surprise"
...
async function loadExternalData() {
    try {
        // TOUJOURS essayer de charger depuis GitHub en priorité
        let barsLoaded = false;
        let filtersLoaded = false;
        
        // Tentative de chargement des bars depuis bars.json (GitHub)
        try {
            const barsResponse = await fetch('bars.json?t=' + Date.now()); // Cache-busting
            if (barsResponse.ok) {
                ...
                saveBars();
                barsLoaded = true;
                console.log('✅ Bars chargés depuis GitHub (bars.json)');
            }
        } catch (fetchError) {
            console.warn('⚠️ Impossible de charger bars.json depuis GitHub:', fetchError.message);
        }
        
        // Si échec GitHub, charger depuis localStorage
        if (!barsLoaded) {
            const savedBars = localStorage.getItem('soif_bars');
            if (savedBars) {
                bars = JSON.parse(savedBars);
                bars = sortBarsByName(bars);
                console.log('📦 Bars chargés depuis localStorage (backup)');
                showNotification('⚠️ Mode hors ligne : données locales utilisées', 'error');
            } else {
                bars = [];
                showNotification('⚠️ Aucune donnée disponible', 'error');
            }
        }
        
        // Charger filters.json depuis GitHub EN PRIORITÉ
        try {
            const filtersResponse = await fetch('filters.json?t=' + Date.now()); // Cache-busting
            if (filtersResponse.ok) {
                FILTERS = await filtersResponse.json();
                filtersLoaded = true;
                console.log('✅ Filtres chargés depuis GitHub (filters.json)');
            }
        } catch (fetchError) {
            console.warn('⚠️ Impossible de charger filters.json depuis GitHub:', fetchError.message);
        }
        
        // Fallback filters depuis localStorage si GitHub échoue
        if (!filtersLoaded) {
            const savedFilters = localStorage.getItem('soif_filters_backup');
            if (savedFilters) {
                FILTERS = JSON.parse(savedFilters);
                console.log('📦 Filtres chargés depuis localStorage (backup)');
            } else {
                // Dernier fallback : structure vide
                FILTERS = {
                ...
                console.log('📦 Filtres par défaut (fallback)');
            }
        } else {
            // Sauvegarder les filtres GitHub dans localStorage comme backup
            localStorage.setItem('soif_filters_backup', JSON.stringify(FILTERS));
        }
...
function init() {
    mergeCustomFilters(); // Fusionner les filtres custom APRÈS le chargement GitHub
    createStickyResultsBanner(); // Créer le bandeau sticky des résultats
    generateFormFilters();
    generateSearchFilters();
    updateCounts();
    performSearch();
    updateBackButton(); // Initialiser l'état du bouton back
    updateSurpriseMeButton(); // Initialiser l'état du bouton "Surprends-moi"
}
...
function setupEventListeners() {
    ...
    document.getElementById('surpriseMeBtn').addEventListener('click', toggleSurpriseMe);
    
    const searchInput = document.getElementById('searchInput');
    searchInput.addEventListener('input', performSearch);
}

// Créer le bandeau sticky des résultats
function createStickyResultsBanner() {
    // Vérifier si le bandeau n'existe pas déjà
    if (document.getElementById('stickyResultsBanner')) return;
    
    const banner = document.createElement('div');
    banner.id = 'stickyResultsBanner';
    banner.className = 'sticky-results-banner';
    banner.innerHTML = `
        <div class="sticky-banner-content">
            <span class="results-text">Résultats trouvés :</span>
            <span class="results-count" id="stickyResultCount">0</span>
            <button class="btn-show-results" id="stickyShowResultsBtn">
                👁️ Voir les bars
            </button>
        </div>
    `;
    ...
    // Ajouter l'écouteur pour le bouton
    document.getElementById('stickyShowResultsBtn').addEventListener('click', showResults);
}
...
function loadCustomFilters() {
    try {
        ...
        if (savedFilters) {
            ...
            console.log('📦 Filtres personnalisés chargés depuis localStorage');
            // Ne pas fusionner ici - on le fera après le chargement de GitHub
        }
    } catch (error) {
...
function mergeCustomFilters() {
    // Fusionner les filtres personnalisés AVEC les filtres de GitHub
    // Uniquement si des filtres custom existent
    if (Object.keys(customFilters).length > 0) {
        ...
        console.log('🔄 Filtres personnalisés fusionnés avec GitHub');
    }
}
...
async function performReset() {
    try {
        ...
        // Réinitialiser la recherche ET le mode surprise
        selectedSearchFilters = [];
        isSurpriseMode = false;
        document.getElementById('searchInput').value = '';
        updateSearchFilterUI();
        updateSelectedFilters();
        updateSurpriseMeButton();
...
function performSearch() {
    ...
    // Mettre à jour le compteur du bandeau sticky
    const stickyCount = document.getElementById('stickyResultCount');
    if (stickyCount) {
        stickyCount.textContent = filtered.length;
    }
...
function backToFilters() {
    // Retirer le dernier état si on revient depuis les résultats
    if (navigationHistory.length > 0 && navigationHistory[navigationHistory.length - 1] === 'filters') {
...
// NOUVELLE FONCTION : Toggle mode surprise
function toggleSurpriseMe() {
    ...
    if (isSurpriseMode) {
        // Désactiver le mode surprise : montrer tous les bars
        isSurpriseMode = false;
        selectedSearchFilters = [];
        document.getElementById('searchInput').value = '';
        updateSearchFilterUI();
        ...
        updateSurpriseMeButton();
        performSearch();
        showNotification('👀 Tous les bars affichés !');
        
        if (typeof gtag !== 'undefined') {
            gtag('event', 'surprise_deactivated', {
                'event_category': 'Recherche',
                'value': 1
            });
        }
    } else {
        // Activer le mode surprise : afficher un bar aléatoire
        isSurpriseMode = true;
...
// NOUVELLE FONCTION : Mettre à jour le bouton "Surprends-moi"
function updateSurpriseMeButton() {
    const btn = document.getElementById('surpriseMeBtn');
    if (isSurpriseMode) {
        btn.textContent = `👀 Voir tous les bars (${bars.length})`;
...
// Ouvrir Google Maps pour un bar
function openGoogleMaps(bar) {
    // Encoder le nom et l'adresse pour l'URL
    const query = encodeURIComponent(`${bar.name}, ${bar.address}`);
    
    // URL Google Maps (fonctionne sur mobile et desktop)
    const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${query}`;
    
    // Ouvrir dans un nouvel onglet
    window.open(mapsUrl, '_blank');
    
    // Analytics (si vous voulez tracker)
    if (typeof gtag !== 'undefined') {
        gtag('event', 'open_maps', {
            'event_category': 'Navigation',
            'event_label': bar.name,
            'value': 1
        });
    }
}
...
function createBarCard(bar, withModifyButton = false) {
    ...
    if (withModifyButton) {
        ...
        mapsBtn.textContent = '📍 Voir sur Maps';
...
function cancelEdit() {
    // Retirer le dernier état si on annule depuis le formulaire
    if (navigationHistory.length > 0 && navigationHistory[navigationHistory.length - 1] === 'main') {
        navigationHistory.pop();
        updateBackButton();
    }
...
// Gestion du bouton retour et historique de navigation
function updateBackButton() {
    const backButton = document.getElementById('backButton');
    const resultsVisible = document.getElementById('resultsSection').style.display !== 'none';
    const formVisible = document.getElementById('barFormContainer').style.display !== 'none';
    
    // Afficher le bouton si on est dans les résultats ou le formulaire
    if (resultsVisible || formVisible || navigationHistory.length > 0) {
...
function goBack() {
    if (navigationHistory.length === 0) {
        // Si pas d'historique, vérifier si on est dans les résultats ou le formulaire
        const resultsVisible = document.getElementById('resultsSection').style.display !== 'none';
        const formVisible = document.getElementById('barFormContainer').style.display !== 'none';
    ...
    if (previousState === 'filters') {
        backToFilters();
    } else if (previousState === 'main') {
        // Retour à la vue principale (fermer formulaire)
        cancelEdit();
    }
}

// Modifier les fonctions existantes pour gérer l'historique
